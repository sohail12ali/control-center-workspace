...
The **app icon** — the exe, taskbar and browser-tab mark, a blue sparkle with a
smaller purple one — is `src-tauri/icons/{32x32,128x128}.png` + `icon.ico`,
generated by `desktop/icons/gen_app_icons.py` from the committed artwork at
`desktop/icons/source/app-icon-512.png`. Change the mark by replacing that one
file and re-running the script; nothing else knows what it looks like. The .ico
carries every size Windows asks for (16/20/24/32/40/48/64/256), each resampled
from the 512px source at that size rather than scaled from a neighbour, which
is what keeps small renderings sharp. The same script writes
`console/static/icon-{16,32,64}.png` so the console's favicon is the same asset.

The source was downloaded from `img.icons8.com/color/512/bard--v2.png`
(Icons8). **Its free tier requires attribution** — if this ships without a paid
licence, that credit is owed somewhere visible.
Regenerate and commit the output; `build.rs` marks the icons as build inputs, so
a rebuild picks them up instead of re-embedding the previous mark.
