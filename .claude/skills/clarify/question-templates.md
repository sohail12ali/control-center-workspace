...
All questions for ticket `{T}` live in `knowledge-center/artifacts/{T}/{T}-questions.toml`, mutated only via `console/kanban.py tracker ... {T} questions ...` (see `questions/SKILL.md`) — never hand-edited.
...
1. Write a question to `{T}-questions.toml` (via `console/kanban.py tracker add {T} questions ...`) when ambiguity blocks a stage or the decision needs an audit trail.
...
- If the user gave verbatim policy text, preserve it when logging to `{T}-questions.toml`.

## Continuation rule

**Pending questions** = unresolved items in `{T}-questions.toml` (`open`/`answered`, or `critical` and not `resolved`) or any CLARIFY-stage item still needing a decision.
