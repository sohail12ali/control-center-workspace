...
# Requirements Draft: T-019

> Working requirements document. **Not frozen.** Expect revisions each iteration until `requirements T-019 freeze` passes.

**Command reference:**
- **Created by:** `requirements T-019 draft`
- **Grounded by:** `analyze T-019` → writes `T-019-context-snapshot.md`
- **Gaps surfaced by:** `challenge-requirements T-019 (gaps dimension)`
- **Challenged by:** `challenge-requirements T-019` (adds ⚠ markers below)
- **Enriched by:** `requirements T-019 enrich [source]`
- **Cross-checked by:** `challenge-requirements T-019 (overlap/conflict/reuse dimension)`
- **Iterated by:** `requirements T-019 iterate "feedback"`
- **Frozen by:** `requirements T-019 freeze` → produces `T-019-requirements-summary.md`
...
(Condensed from [[T-019-context-snapshot]])
...
(Populated by `challenge-requirements T-019 (overlap/conflict/reuse dimension)`)
...
Mirrored from `T-019-questions.toml` (`console/kanban.py tracker list T-019 questions`). Blocker questions must be resolved before freeze.

- Q{n}: _text_ — status: open | answered | resolved

## 13. Challenge Findings (⚠)

(Appended by `challenge-requirements T-019`. Each must be resolved or explicitly accepted before freeze.)

- ⚠ _finding_ — **resolution:** 〈TBD〉

## 14. Draft History

See [[T-019-iteration-log]] for per-iteration diff + rationale.
...
- [ ] `T-019-requirements-summary.md` generated for `requirements stories` consumption
