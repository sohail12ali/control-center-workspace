...
# Requirements Draft: T-011

> Working requirements document. **Not frozen.** Expect revisions each iteration until `requirements T-011 freeze` passes.

**Command reference:**
- **Created by:** `requirements T-011 draft`
- **Grounded by:** `analyze T-011` → writes `T-011-context-snapshot.md`
- **Gaps surfaced by:** `challenge-requirements T-011 (gaps dimension)`
- **Challenged by:** `challenge-requirements T-011` (adds ⚠ markers below)
- **Enriched by:** `requirements T-011 enrich [source]`
- **Cross-checked by:** `challenge-requirements T-011 (overlap/conflict/reuse dimension)`
- **Iterated by:** `requirements T-011 iterate "feedback"`
- **Frozen by:** `requirements T-011 freeze` → produces `T-011-requirements-summary.md`
...
(Condensed from [[T-011-context-snapshot]])
...
(Populated by `challenge-requirements T-011 (overlap/conflict/reuse dimension)`)
...
Mirrored from `T-011-questions.toml` (`console/kanban.py tracker list T-011 questions`). Blocker questions must be resolved before freeze.

- Q{n}: _text_ — status: open | answered | resolved

## 13. Challenge Findings (⚠)

(Appended by `challenge-requirements T-011`. Each must be resolved or explicitly accepted before freeze.)

- ⚠ _finding_ — **resolution:** 〈TBD〉

## 14. Draft History

See [[T-011-iteration-log]] for per-iteration diff + rationale.
...
- [ ] `T-011-requirements-summary.md` generated for `requirements stories` consumption
