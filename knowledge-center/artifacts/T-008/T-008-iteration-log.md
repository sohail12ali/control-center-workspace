...
- **Next recommended:** `analyze T-008` if not run, then `challenge-requirements T-008 (gaps dimension)`
...
|---|---|---|---|---|
| | | | | `requirements T-008 freeze` |

---

## Rollback

To see the draft at a past iteration, use `git log` on `T-008-requirements-draft.md`. The draft is mutable by design — this log plus git history is the source of truth.
