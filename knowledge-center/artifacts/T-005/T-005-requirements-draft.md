...
# Requirements Draft: T-005

> Working requirements document. **Not frozen.** Expect revisions each iteration until `requirements T-005 freeze` passes.

**Command reference:**
- **Created by:** `requirements T-005 draft`
- **Grounded by:** `analyze T-005` → writes `T-005-context-snapshot.md`
- **Gaps surfaced by:** `challenge-requirements T-005 (gaps dimension)`
- **Challenged by:** `challenge-requirements T-005` (adds ⚠ markers below)
- **Enriched by:** `requirements T-005 enrich [source]`
- **Cross-checked by:** `challenge-requirements T-005 (overlap/conflict/reuse dimension)`
- **Iterated by:** `requirements T-005 iterate "feedback"`
- **Frozen by:** `requirements T-005 freeze` → produces `T-005-requirements-summary.md`
...
(Condensed from [[T-005-context-snapshot]])
...
(Populated by `challenge-requirements T-005 (overlap/conflict/reuse dimension)`)
...
Mirrored from `T-005-questions.toml` (`console/kanban.py tracker list T-005 questions`). Blocker questions must be resolved before freeze.

- Q{n}: _text_ — status: open | answered | resolved

## 13. Challenge Findings (⚠)

(Appended by `challenge-requirements T-005`. Each must be resolved or explicitly accepted before freeze.)

- ⚠ _finding_ — **resolution:** 〈TBD〉

## 14. Draft History

See [[T-005-iteration-log]] for per-iteration diff + rationale.
...
- [ ] `T-005-requirements-summary.md` generated for `requirements stories` consumption
