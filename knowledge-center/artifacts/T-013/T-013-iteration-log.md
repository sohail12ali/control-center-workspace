...
- **Next recommended:** `analyze T-013` if not run, then `challenge-requirements T-013 (gaps dimension)`
...
|---|---|---|---|---|
| | | | | `requirements T-013 freeze` |

---

## Rollback

To see the draft at a past iteration, use `git log` on `T-013-requirements-draft.md`. The draft is mutable by design — this log plus git history is the source of truth.
