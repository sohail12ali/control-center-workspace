...
# Requirements Draft: T-007

> Working requirements document. **Not frozen.** Expect revisions each iteration until `requirements T-007 freeze` passes.

**Command reference:**
- **Created by:** `requirements T-007 draft`
- **Grounded by:** `analyze T-007` → writes `T-007-context-snapshot.md`
- **Gaps surfaced by:** `challenge-requirements T-007 (gaps dimension)`
- **Challenged by:** `challenge-requirements T-007` (adds ⚠ markers below)
- **Enriched by:** `requirements T-007 enrich [source]`
- **Cross-checked by:** `challenge-requirements T-007 (overlap/conflict/reuse dimension)`
- **Iterated by:** `requirements T-007 iterate "feedback"`
- **Frozen by:** `requirements T-007 freeze` → produces `T-007-requirements-summary.md`
...
(Condensed from [[T-007-context-snapshot]])
...
(Populated by `challenge-requirements T-007 (overlap/conflict/reuse dimension)`)
...
Mirrored from `T-007-questions.toml` (`console/kanban.py tracker list T-007 questions`). Blocker questions must be resolved before freeze.

- Q{n}: _text_ — status: open | answered | resolved

## 13. Challenge Findings (⚠)

(Appended by `challenge-requirements T-007`. Each must be resolved or explicitly accepted before freeze.)

- ⚠ _finding_ — **resolution:** 〈TBD〉

## 14. Draft History

See [[T-007-iteration-log]] for per-iteration diff + rationale.
...
- [ ] `T-007-requirements-summary.md` generated for `requirements stories` consumption
