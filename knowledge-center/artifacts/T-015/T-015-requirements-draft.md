...
# Requirements Draft: T-015

> Working requirements document. **Not frozen.** Expect revisions each iteration until `requirements T-015 freeze` passes.

**Command reference:**
- **Created by:** `requirements T-015 draft`
- **Grounded by:** `analyze T-015` → writes `T-015-context-snapshot.md`
- **Gaps surfaced by:** `challenge-requirements T-015 (gaps dimension)`
- **Challenged by:** `challenge-requirements T-015` (adds ⚠ markers below)
- **Enriched by:** `requirements T-015 enrich [source]`
- **Cross-checked by:** `challenge-requirements T-015 (overlap/conflict/reuse dimension)`
- **Iterated by:** `requirements T-015 iterate "feedback"`
- **Frozen by:** `requirements T-015 freeze` → produces `T-015-requirements-summary.md`
...
(Condensed from [[T-015-context-snapshot]])
...
(Populated by `challenge-requirements T-015 (overlap/conflict/reuse dimension)`)
...
Mirrored from `T-015-questions.toml` (`console/kanban.py tracker list T-015 questions`). Blocker questions must be resolved before freeze.

- Q{n}: _text_ — status: open | answered | resolved

## 13. Challenge Findings (⚠)

(Appended by `challenge-requirements T-015`. Each must be resolved or explicitly accepted before freeze.)

- ⚠ _finding_ — **resolution:** 〈TBD〉

## 14. Draft History

See [[T-015-iteration-log]] for per-iteration diff + rationale.
...
- [ ] `T-015-requirements-summary.md` generated for `requirements stories` consumption
