...
# Requirements Draft: T-006

> Working requirements document. **Not frozen.** Expect revisions each iteration until `requirements T-006 freeze` passes.

**Command reference:**
- **Created by:** `requirements T-006 draft`
- **Grounded by:** `analyze T-006` → writes `T-006-context-snapshot.md`
- **Gaps surfaced by:** `challenge-requirements T-006 (gaps dimension)`
- **Challenged by:** `challenge-requirements T-006` (adds ⚠ markers below)
- **Enriched by:** `requirements T-006 enrich [source]`
- **Cross-checked by:** `challenge-requirements T-006 (overlap/conflict/reuse dimension)`
- **Iterated by:** `requirements T-006 iterate "feedback"`
- **Frozen by:** `requirements T-006 freeze` → produces `T-006-requirements-summary.md`
...
(Condensed from [[T-006-context-snapshot]])
...
(Populated by `challenge-requirements T-006 (overlap/conflict/reuse dimension)`)
...
Mirrored from `T-006-questions.toml` (`console/kanban.py tracker list T-006 questions`). Blocker questions must be resolved before freeze.

- Q{n}: _text_ — status: open | answered | resolved

## 13. Challenge Findings (⚠)

(Appended by `challenge-requirements T-006`. Each must be resolved or explicitly accepted before freeze.)

- ⚠ _finding_ — **resolution:** 〈TBD〉

## 14. Draft History

See [[T-006-iteration-log]] for per-iteration diff + rationale.
...
- [ ] `T-006-requirements-summary.md` generated for `requirements stories` consumption
